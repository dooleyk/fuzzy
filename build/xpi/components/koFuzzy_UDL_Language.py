# Registers the Fuzzy language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koFuzzyLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Fuzzy")
    registry.registerLanguage(KoFuzzyLanguage())

class KoFuzzyLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Fuzzy"
    lexresLangName = "Fuzzy"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "ece2f47d-f9f7-4d3d-8c9e-8ebc02311abf"

    defaultExtension = '.fzy'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Fuzzy'}
